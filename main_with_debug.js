...
//Added debugAjax to initialize the debug.
function initialize() {
  cities();
  debugAjax();
};
...
function debugCallback(response) {
  document.getElementById("mydiv").insertAdjacentHTML('beforeend', 'GeoJSON data: ' + JSON.stringify(response))
};

//Turned string into JSON so the function could be read and printed. 
//Created a second callback in order to read the file as a JSON. This
//Also prints out the function on the HTML.
function debugAjax() {
...
//Deleted lines 14-17 on the debug ajax code since the function can run more
//fluently with the first statement.